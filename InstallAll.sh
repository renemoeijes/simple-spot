...
install_mybluetooth(){
   printf "Installing Simple Bleutooth Agent (ALSA)"
    ...
    # Download en installeer het laatste simple-bt DEB-pakket van GitHub
    TMP_DEB="/tmp/simple-bt-latest.deb"
    echo "Downloading latest simple-bt release..."
    wget -O "$TMP_DEB" $(curl -s https://api.github.com/repos/renemoeijes/simple-bt/releases/latest | jq -r '.assets[] | select(.name | endswith(".deb")) | .browser_download_url')
    echo "Installing simple-bt..."
    sudo dpkg -i "$TMP_DEB"
    rm -f "$TMP_DEB"
    echo "simple-bt is installed."
}
